...
const NodeCanvas: React.FC<NodeCanvasProps> = ({
  ...
  const getTransitionsBetweenStates = (sourceId: string, targetId: string): { inputSymbol: string, outputSymbol?: string }[] => {
    ...
    // Return input and output symbols
  ...
  const drawSelfLoop = (x: number, y: number, nodeId: string, index: number | string, isHighlighted: boolean) => {
    ...
    // Get input and output symbols for self-transitions
    const transitionLabels = selfTransitions.map(t => {
      if (t.outputSymbol) {
        return `${t.inputSymbol}/${t.outputSymbol}`;
      }
      return t.inputSymbol;
    });
    
    // If there are too many labels, show a counter instead
    const showCounter = transitionLabels.length > 4 && !isHighlighted && !isTransitionLabelVisible(nodeId, nodeId);
    
    // Calculate dimensions for the label background
    const lineHeight = 16;
    const totalHeight = showCounter ? lineHeight : transitionLabels.length * lineHeight;
    const maxLabelWidth = showCounter ? 60 : Math.max(...transitionLabels.map(label => label.length * 8)) + 10;
        ...
        {showCounter ? (
          <>
            ...
            <Text
              ...
              width={60}
              text={`${selfTransitions.length} transitions`}
              fontSize={12}
              fill="black"
              align="center"
    ...
    <>
      ...
      {nodes.map((node, index) => {
        ...
        return node.transitions.map((transition, tindex) => {
          ...
          // Regular transition
            ...
            // Get all transition input symbols between these nodes
            const inputSymbols = getTransitionsBetweenStates(sourceId, targetId);
            ...
            const showCounter = inputSymbols.length > 4 && !isTransitionHighlighted(sourceId, targetId) && !showLabels;
                ...
                {showCounter ? (
                  ...
                  <Group perfectDrawEnabled={false}>
                    ...
                    <Text
                      ...
                      width={60}
                      text={`${inputSymbols.length} inputs`}
                      fontSize={12}
                      fill="black"
                      align="center"
                      verticalAlign="middle"
                  ...
                  showLabels && (
                    <Group perfectDrawEnabled={false}>
                      {(() => {
                        ...
                        const lineHeight = 18;
                        const totalHeight = inputSymbols.length * lineHeight;
                        
                        // Format the labels based on the transition
                        const labelTexts = inputSymbols.map(t => {
                          if (t.outputSymbol) {
                            return `${t.inputSymbol}/${t.outputSymbol}`;
                          }
                          return t.inputSymbol;
                        ...
                        const maxLength = Math.max(...labelTexts.map(lbl => lbl.length));
                        const boxWidth = maxLength * 8 + 10;
                        ...
                        return (
                          <>
                            ...
                            {/* Render each input symbol */}
                            {labelTexts.map((label, i) => (
                              <Text
                                key={i}
                                x={baseX + boxWidth / 2 - (label.length * 4)}
      ...
      {nodes.map((node) => (
        ...
        >
          ...
          {/* State output value */}
          <Text
            x={0 - (node.output?.length || 1) * 5 + 10}
            y={4}
            text={node.output || "ε"}
            fill={currNodes.has(node.id) ? "white" : "blue"}
            fontSize={14}
            fontStyle="italic"
            align="center"
            verticalAlign="middle"
