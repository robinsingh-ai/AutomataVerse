...
import { ControlPanelProps, MachineType } from '../type';
...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  return (
    <DraggablePanel title="FSM Control" defaultPosition={{ x: 20, y: 130 }}>
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {/* Only show state output button for Moore machines */}
          {machineType === 'Moore' && selectedNode && onSetStateOutput && (
            <button
              onClick={() => onSetStateOutput(selectedNode)}
              className={`w-full flex items-center justify-center gap-2 py-2 px-4 rounded transition-colors ${
                isDark
                  ? 'bg-gray-800 hover:bg-gray-700 text-white'
                  : 'bg-white hover:bg-gray-100 text-gray-800 border border-gray-300'
            ...
            >
              ...
              <span>Set State Output</span>
            </button>
          )}
          
          {/* Machine type toggle only when not in problem mode */}
          {!problemMode && onMachineTypeChange && (
            <div className={`mt-4 p-3 rounded ${isDark ? 'bg-gray-800' : 'bg-gray-50 border border-gray-200'}`}>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => onMachineTypeChange('Moore')}
                ...
                <button
                  onClick={() => onMachineTypeChange('Mealy')}
                  className={`flex-1 py-1.5 px-3 text-sm rounded transition-colors ${
                    machineType === 'Mealy'
                      ? isDark 
                        ? 'bg-blue-600 text-white' 
